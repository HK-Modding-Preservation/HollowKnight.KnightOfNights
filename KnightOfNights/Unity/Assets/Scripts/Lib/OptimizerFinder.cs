...
namespace KnightOfNights.Scripts.Lib
{
    ...
    public class OptimizerFinder
    {
        ...
        private static bool FixSDOs()
        {
            ...
            foreach (var sdo in Object.FindObjectsOfType<SceneDataOptimizer>(true))
                map.GetOrAddNew(sdo.Priority).Add(sdo);
            ...
            foreach (var key in keys)
            {
                foreach (var sdo in map[key])
                {
                    ...
                    if (result)
                        UnityEditorShims.MarkDirty(sdo);
                }
            }
            ...
            foreach (var component in Object.FindObjectsOfType<Component>(true))
            {
                ...
                changed |= result;
                if (result)
                    UnityEditorShims.MarkDirty(component);
            }
            return changed;
        }
        ...
        private static List<object> GetSceneData()
        {
            ...
            foreach (
                var provider in GameObjectExtensions
                    .GetComponentsInScene<SceneDataProvider>(true)
                    .OrderBy(p => p.name)
            )
        ...
        private static readonly Dictionary<string, int> SortingLayers = new Dictionary<
            string,
            int
        >()
        {
        ...
        private static void ParseOrdinal(string name, out string prefix, out int ordinal)
        {
            if (name.EndsWith(")"))
            {
                ...
                if (idx > 0)
                {
                    ...
                    if (int.TryParse(substr, out ordinal))
                        return;
                }
            }
        ...
        private static string PrintOrdinal(string prefix, int ordinal) =>
            ordinal == 0 ? prefix : $"{prefix} ({ordinal})";
