...
internal class MarkothShieldWave : MonoBehaviour
{
    ...
    public static MarkothShieldWave Spawn(
        ShieldCycloneStats stats,
    ...
    )
    {
        ...
        for (int i = 0; i < stats.ShieldsPerWave; i++)
        {
            ...
            shield.transform.localRotation = Quaternion.Euler(
                0,
                0,
                180f + (i * 360f) / stats.ShieldsPerWave
            );
    ...
    {
        float s = flipped ? -1 : 1;
        expansion.SimpleAccelerate(
            ref expansionSpeed,
            stats!.ExpansionTopSpeed,
            stats.ExpansionAccel,
            Time.deltaTime
        );
        rotation.SimpleDecelerate(
            ref rotationSpeed,
            stats.RotationSpeedMinimum,
            stats.RotationSpeedDecel,
            Time.deltaTime
        );

        SetPositions();
        if (expansion > stats.ExpansionLimit)
            Despawn();
    }
    ...
    private void SetPositions()
    {
        ...
        for (int i = 0; i < shields.Count; i++)
            shields[i].transform.localPosition =
                Quaternion.Euler(0, 0, (i * 360f) / shields.Count) * new Vector3(expansion, 0);
    }
}
