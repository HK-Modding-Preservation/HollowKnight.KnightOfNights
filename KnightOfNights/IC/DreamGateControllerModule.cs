...
internal class DreamGateControllerModule : AbstractModule<DreamGateControllerModule>
{
    ...
    protected override void InitializeInternal() =>
        ItemChanger.Events.AddFsmEdit(dreamnailId, ModifyDreamnail);

    protected override void UnloadInternal() =>
        ItemChanger.Events.RemoveFsmEdit(dreamnailId, ModifyDreamnail);

    private void ModifyDreamnail(PlayMakerFSM fsm)
    {
        fsm.GetState("Can Warp?")
            .AddFirstAction(
                new Lambda(() =>
                {
                    if (PlayerData.instance.GetInt(nameof(PlayerData.dreamOrbs)) <= 0)
                        fsm.SendEvent("NO ESSENCE");
                    else if (!CanWarp())
                        fsm.SendEvent("FAIL");
                })
            );
    ...
    private bool CanCharmWarp()
    {
        ...
        foreach (
            var forbid in SceneDataModule.Get()?.GetForScene<ForbidCharmWarp>(targetScene) ?? []
        )
        {
            if (forbid.WholeScene)
                return false;
            if (
                pos.x >= forbid.MinX
                && pos.x <= forbid.MaxX
    ...
    {
        if (CharmIds.EquippedAnyCharmsBesidesVoidHeart() && !CanCharmWarp())
            return false;
    ...
    private bool CanSet()
    {
        ...
        if (
            WindField
                .ActiveWindEffects(HeroController.instance.transform.position, WindTargetType.Hero)
                .sqrMagnitude > 0
        )
            return false;
