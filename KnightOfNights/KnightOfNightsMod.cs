...
public class KnightOfNightsMod : Mod, IGlobalSettings<GlobalSettings>
{
    ...
    private static readonly string VERSION =
        PurenailCore.ModUtil.VersionUtil.ComputeVersion<KnightOfNightsMod>();

    public override string GetVersion() => VERSION;

    public KnightOfNightsMod()
        : base("KnightOfNights")
    {
        Instance = this;
    ...
    public override List<(string, string)> GetPreloadNames() =>
        KnightOfNightsPreloader.Instance.GetPreloadNames();

    public override (string, Func<IEnumerator>)[] PreloadSceneHooks() =>
        KnightOfNightsPreloader.Instance.PreloadSceneHooks();
