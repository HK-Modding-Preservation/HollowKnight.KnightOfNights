...
public class DarknessRegion : MonoBehaviour
{
    ...
    private static int? ComputeDarkness()
    {
        ...
        foreach (var control in activeControls)
        {
            ...
            if (darkness == null || d > darkness.Value)
                darkness = d;
        }
        return darkness;
    }
    ...
    private static void UpdateDarkness()
    {
        ...
        if (newDarkness != prevDarkness)
        {
            ...
            var fsm = HeroController
                .instance.gameObject.FindChild("Vignette")
                .LocateMyFSM("Darkness Control");
            fsm.FsmVariables.GetFsmInt("Darkness Level").Value =
                newDarkness ?? sceneManager.darknessLevel;
    ...
    {
        if (activeControls.Add(this))
            UpdateDarkness();
    }

    private void Deactivate()
    {
        if (activeControls.Remove(this))
            UpdateDarkness();
    }
